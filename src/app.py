...
with tab_search:
    ...
    if search_clicked and query.strip():
        ...
        for r in results:
            ...
            if show_keywords and "keywords" in r:
                try:
                    from search_engine import extract_entities
                    entities = extract_entities(r["abstract"])
                    if entities:
                        st.markdown("**🏷️ Named Entities:**")
                        for label, items in entities.items():
                            st.markdown(f"`{label}`: {', '.join(items[:3])}")
                except:
                    pass
...
with tab_resume:
    ...
    if analyze_clicked and uploaded_file is not None:
        ...
        st.caption("Based on your skills, these ML/AI roles suit your profile:")
        from resume_analyzer import match_job_roles
        matched_jobs = match_job_roles(report["raw_text"])
