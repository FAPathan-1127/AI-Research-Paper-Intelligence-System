...
ML_AI_JOB_ROLES = [
    ...
    "MLOps Engineer", "Data Engineer", "Research Scientist",
    "AI Research Engineer", "Business Intelligence Analyst",
]

def match_job_roles(resume_text: str) -> list:
    """Match resume with relevant ML/AI job roles."""
    text_lower = resume_text.lower()
    matched = []
    skill_count = len([s for s in ML_AI_SKILLS if s in text_lower])
    
    if skill_count >= 10:
        matched = ML_AI_JOB_ROLES[:5]
    elif skill_count >= 6:
        matched = ML_AI_JOB_ROLES[:3]
    else:
        matched = ML_AI_JOB_ROLES[:2]
    return matched
